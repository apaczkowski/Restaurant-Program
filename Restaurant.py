...
def insert_product():
    if request.method == 'POST':
        ...
        conn.execute("INSERT INTO Product (ProductName, MenuPrice, Calories, Points, FoodType, Menu) VALUES (?, ?, ?, ?, ?, ?)",
                     (product_name, menu_price, calories, points, food_type, menu))
        conn.commit()
        conn.close()
...
def report_products():
    query = "SELECT p.ProductID, p.ProductName, p.RestaurantCode, p.MenuPrice, p.Calories, p.Points, p.FoodType, p.Menu, ROUND(r.restaurantpcf * (MenuPrice / Points * 10000), 4) AS PercentReturn, ROUND((Calories / MenuPrice), 4) AS CaloriesPerDollar, ROUND((Calories / Points), 4) AS CaloriesPerPoint FROM Product p, restaurants r WHERE 1=1 and p.restaurantcode = r.restaurantcode"
    params = []

    if request.method == 'POST':
        # Transferring from webpage to python variables
        ...
        calories_per_dollar = request.form.get('calories_per_dollar')
        calories_per_point = request.form.get('calories_per_point')

        # Transferring from python variables to database
        if product_name:
        ...
        if calories:
            min_calories, max_calories = map(float, points.split(','))
            query += " AND Calories BETWEEN ? AND ?"
        ...
        if calories_per_dollar:
            min_value, max_value = map(float, price_to_calories.split(','))
            query += " AND CaloriesPerDollar BETWEEN ? AND ?"
            params.append(min_value)
            params.append(max_value)
        if calories_per_point:
            min_value, max_value = map(float, price_to_calories.split(','))
            query += " AND CaloriesPerPoint BETWEEN ? AND ?"
            params.append(min_value)
            params.append(max_value)
...
def report_deals():
    query = '''
        SELECT Deals.DealID, Deals.DealName, Deals.DealPrice, Product.ProductName, Product.MenuPrice, Product.Calories
        FROM Deals
    ...
    for deal in deals:
        if current_deal and current_deal['DealID'] != deal['DealID']:
            ...
            total_individual_calories = sum([q['Calories'] for q in current_deal['Products'] if q['Calories'] is not None])
            ...
            caloriesPerDollar = (total_individual_calories / current_deal['DealPrice'])
            ...
            current_deal['CaloriesPerDollar'] = caloriesPerDollar
...
def update_product():
    if request.method == 'POST':
        ...
        conn.execute("""
            UPDATE Product
            SET ProductName = ?, MenuPrice = ?, Calories = ?, Points = ?, FoodType = ?, Menu = ?
            WHERE ProductID = ?
        """, (product_name, menu_price, calories, points, food_type, menu, product_id))
        conn.commit()
        conn.close()
        return redirect(url_for('report_products'))
